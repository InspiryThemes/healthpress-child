...
if ( ! function_exists( 'inspiry_enqueue_child_styles' ) ) {
	...
	function inspiry_enqueue_child_styles() {
		...
		if ( ! is_admin() ) {
			...
			// child default css
			wp_enqueue_style( 'child-default', get_stylesheet_uri(), array( 'parent-default' ), '1.6.0', 'all' );
...
if ( ! function_exists( 'inspiry_load_translation_from_child' ) ) {
	...
	function inspiry_load_translation_from_child() {
		load_child_theme_textdomain( 'healthpress-child', get_stylesheet_directory() . '/languages' );
	}
